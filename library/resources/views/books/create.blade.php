    ...
    <div class="flex justify-center items-center min-h-screen py-8">
        <div class="bg-white shadow-lg rounded-lg w-full max-w-xl">
            <div class="bg-blue-600 text-white text-center py-3 px-6 rounded-t-lg">
                <h3 class="text-xl font-bold">Add New Book</h3>
            </div>
            <div class="px-6 py-3">
                <form method="POST" action="{{ route('books.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text"
                            ...
                            id="name" name="name" required value="{{ old('name') }}" placeholder="Enter book title"
                            required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-gray-500 text-xs mt-1">You can upload a cover image (optional)</p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number"
                            ...
                            id="isbn" name="isbn" value="{{ old('isbn') }}" placeholder="Enter ISBN (13 digits)"
                            required pattern="[0-9]{13}" inputmode="numeric" min="0" maxlength="13"
                            oninput="javascript: if (this.value.length > 13) this.value = this.value.slice(0, 13);" re>
                        @error('isbn')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select
                            ...
                            id="author_ids" name="author_ids[]" multiple required style="height: 120px;" required>
                            @foreach($authors as $author)
                            <option value="{{ $author->id }}" {{ (collect(old('author_ids'))->contains($author->id)) ?
                                'selected' : '' }}>
                                {{ $author->name }}
                            </option>
                            @endforeach
                    ...
                    <div class="mb-4">
                        ...
                        <textarea
                            ...
                            placeholder="Enter bibliography details">{{ old('bibliography') }}</textarea>
                        @error('bibliography')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
